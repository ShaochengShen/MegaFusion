    ...
    def __call__(
        ...
        with self.progress_bar(total=len(stage_timesteps)) as progress_bar:
            for i, t in enumerate(stage_timesteps):
                ...
                latents, z_0_predict = self.scheduler.step(noise_pred, t, latents, return_dict=False)
                
                if latents.dtype != latents_dtype:
                    if torch.backends.mps.is_available():
                        latents = latents.to(latents_dtype)
                        z_0_predict = z_0_predict.to(latents_dtype)
        ...
        if output_type == "latent":
            ...
            z_0_predict = (z_0_predict / self.vae.config.scaling_factor) + self.vae.config.shift_factor

            image = self.vae.decode(latents, return_dict=False)[0]
            image = self.image_processor.postprocess(image, output_type=output_type)
            x_0_predict = self.vae.decode(z_0_predict, return_dict=False)[0]
            x_0_predict = self.image_processor.postprocess(x_0_predict, output_type=output_type)
        ...
        return StableDiffusion3PipelineOutput(images=image), StableDiffusion3PipelineOutput(images=x_0_predict)